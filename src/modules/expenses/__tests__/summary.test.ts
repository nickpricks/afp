...
import { TimeRange, ExpenseCategory, IncomeSource, PaymentMethod } from '@/shared/types';
...
describe('filterByDateRange', () => {
  ...
  it('All returns everything', () => {
    expect(filterByDateRange(expenses, TimeRange.All, '2026-04-07', (e) => e.date)).toHaveLength(5);
  });

  it('Today returns only today', () => {
    const result = filterByDateRange(expenses, TimeRange.Today, '2026-04-07', (e) => e.date);
    expect(result).toHaveLength(1);
    expect(result[0]!.date).toBe('2026-04-07');
  });

  it('Week returns last 7 days', () => {
    const result = filterByDateRange(expenses, TimeRange.Week, '2026-04-07', (e) => e.date);
    // Apr 7, 6, 3 are within 7 days; Mar 15 and Dec 1 are not
    expect(result).toHaveLength(3);
  });

  it('Month returns last 30 days', () => {
    const result = filterByDateRange(expenses, TimeRange.Month, '2026-04-07', (e) => e.date);
  ...
  it('returns empty array for empty input', () => {
    expect(
      filterByDateRange<Expense>([], TimeRange.Today, '2026-04-07', (e) => e.date),
    ).toHaveLength(0);
  });
});
