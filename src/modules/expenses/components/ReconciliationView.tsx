...
export function ReconciliationView({ expenses }: { expenses: Expense[] }) {
  ...
  if (ccCharges.length === 0 && settlements.length === 0) {
    return <p className="px-4 py-8 text-center text-fg-muted">No CC activity</p>;
  }
          ...
          <ul className="flex flex-col gap-1">
            {ccCharges.map((e) => {
              ...
              return (
                <li
                  key={e.id}
                  className="flex justify-between rounded-lg border border-line bg-surface-card px-3 py-2 text-sm"
                >
                  <span className="text-fg">
                    {e.date} {pm?.shortLabel}
                  </span>
