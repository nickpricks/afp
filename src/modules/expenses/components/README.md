...
- **FamilyLedgerTab.tsx** — Read-only Family ledger (Family Umbrella Pillar 2): every member's expenses in one Daily Ledger list with per-member attribution chips (stable palette) + contribution summary strip via `computeFamilyTotals`. Gated on `profile.familyId` in `ExpenseListPage`. No delete/swipe/tap-to-populate — read-only by construction; owner = payer
