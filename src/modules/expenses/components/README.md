...
- **AutoTab.tsx** — Vehicle/Travel filtered tab with `<ServiceDueBanner>`, three quick-add buttons (⛽/🚕/🔧), inline form (tap-to-populate edit), and Daily Ledger list of `<AutoTabRow>` entries. No `<ListControls>` strip — vehicle history is small enough that pagination chrome is overhead.
- **AutoTabRow.tsx** — Single row in the Auto tab list. Renders meta badges (e.g. `⛽ 40L · ₹100/L · 12,300km · 14.5 km/L`) via helpers from `expense-badges.ts`. Old expenses without `meta` show an "incomplete" pill.
