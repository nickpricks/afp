...
type ActivityToggle = {
  key: keyof Pick<BodyConfig, 'floors' | 'walking' | 'running' | 'cycling' | 'yoga'>;
  label: string;
  disabled: boolean;
};
...
const GOAL_SLIDERS = [
  {
    ...
    step: 1,
    unit: '',
    weight: SCORING_WEIGHTS.FLOOR_UP,
    defaultVal: 10,
  },
  {
    key: 'floors' as const,
    ...
    step: 1,
    unit: '',
    weight: SCORING_WEIGHTS.FLOOR_DOWN,
    defaultVal: 5,
  },
  {
    ...
    step: 0.5,
    unit: 'km',
    weight: SCORING_WEIGHTS.WALK_PER_KM,
    defaultVal: 2,
  },
  {
    ...
    step: 0.5,
    unit: 'km',
    weight: SCORING_WEIGHTS.RUN_PER_KM,
    defaultVal: 1,
  },
  {
    ...
    step: 0.5,
    unit: 'km',
    weight: SCORING_WEIGHTS.CYCLE_PER_KM,
    defaultVal: 0,
  },
];
...
const ZONES = [
  {
    max: 20,
    name: 'Easy Start',
    hint: 'A few floors, maybe a short stroll.',
    color: 'text-success',
  },
  {
    max: 40,
    name: 'Light Day',
    hint: 'Some floors and a walk. Gentle nudge.',
    color: 'text-accent',
  },
  {
    max: 65,
    name: 'Solid Day',
    hint: "Good mix of activities. You'll hit this most days.",
    color: 'text-accent',
  },
  {
    max: 90,
    name: 'Push It',
    hint: 'Floors + a walk + a run. You meant it.',
    color: 'text-warning',
  },
  { max: 120, name: 'High Gear', hint: 'Serious activity day.', color: 'text-warning' },
  {
    max: Infinity,
    name: 'Beast Mode',
    hint: "The full package. You're crushing it.",
    color: 'text-error',
  },
];
...
export function BodyConfigForm({
  ...
  const handleSave = async () => {
    const hasAny =
      config.floors || config.walking || config.running || config.cycling || config.yoga;
    if (!hasAny) {
    ...
    <div className="flex flex-col gap-6 px-4 py-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        {ACTIVITY_TOGGLES.map((toggle) => (
          <label
            key={toggle.key}
            className={`flex items-center justify-between rounded-lg bg-surface-card border border-line px-4 py-3 ${
        ...
        <div className="flex items-center justify-center gap-5 py-2">
          <div className="relative" style={{ width: 100, height: 100 }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-[9px] font-semibold text-fg-muted uppercase tracking-wider">
                Goal
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 mt-3 pt-3 border-t border-line">
            {config.floors && (
              <span className="text-[10px] text-fg-muted">
                <span className="font-bold rounded bg-surface px-1.5 py-0.5">
                  ↑×{SCORING_WEIGHTS.FLOOR_UP}
                </span>{' '}
                <span className="font-bold rounded bg-surface px-1.5 py-0.5">
                  ↓×{SCORING_WEIGHTS.FLOOR_DOWN}
                </span>
              </span>
            )}
            {config.walking && (
              <span className="text-[10px] text-fg-muted font-bold rounded bg-surface px-1.5 py-0.5">
                walk km×{SCORING_WEIGHTS.WALK_PER_KM}
              </span>
            )}
            {config.running && (
              <span className="text-[10px] text-fg-muted font-bold rounded bg-surface px-1.5 py-0.5">
                run km×{SCORING_WEIGHTS.RUN_PER_KM}
              </span>
            )}
            {config.cycling && (
              <span className="text-[10px] text-fg-muted font-bold rounded bg-surface px-1.5 py-0.5">
                cycle km×{SCORING_WEIGHTS.CYCLE_PER_KM}
              </span>
            )}
          </div>
        </div>
