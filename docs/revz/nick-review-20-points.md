...
| 11 | **Result\<T\> on async — never void** | **Storage-touching async functions** (anything that calls `adapter.save/remove/get*` or directly hits Firestore/localStorage) return `Result<T>`. Use `ok()`, `err()`, `isOk()`, `isErr()`. `void` return = silent failure waiting to happen. | **Component callback props** that delegate to such functions may return `Promise<void>` IF the hook owns the toast/error path — the prop's contract is "I've handled the failure mode internally." Do not thread `Result<T>` through every layer; thread it where the failure can be acted on. |
...
| 15 | **`package.json` scripts — `&&` is acceptable on POSIX-compatible setups** | `&&` is fine when CI and dev environments are POSIX-compatible (macOS / Linux / WSL — AFP's current target). If Windows PowerShell becomes a target, switch to a Makefile or `concurrently`. Prefer script composition (one script calls others) regardless. | The original "never `&&`" rule applies only when Windows PowerShell is in scope. |
